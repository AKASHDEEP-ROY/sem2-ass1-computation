...
Problem-4.4.1
Akashdeep Roy
Solve a system of linear equations in iterative process using Jacobi method
'''
from numpy import *
...
while(w==1):
  ...
  #initializing p = [0,0,...]  
    p_empty=zeros(5,dtype=float)    
  #doing the algebra
    for i in range(5):
      for j in range(5):
        if j!=i :  
          p_empty[i]=p_empty[i]+((A_1[i][j])*(X1_empty[j]))
